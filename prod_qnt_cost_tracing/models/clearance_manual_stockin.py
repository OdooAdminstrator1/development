...
class ClearanceManualStockin(models.Model):
    _name = "clearance.stockin.manual"
    _auto = False 
...
select row_number() OVER(order by B.move_id) AS id, B.move_id as move_id,sum(B.balance) as balance from stock_product_trace A   inner join 
