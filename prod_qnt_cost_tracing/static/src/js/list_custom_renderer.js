odoo.define('prod_qnt_cost_tracing.ListCustomRenderer', function (require) {
    ...
    custom_function: async function()
    {
        ...
        if (state.model==="clearance.acc.analysis")
        {
            ...
            const ddic={ 'stockin' : 'Stock In Balance','difference' : 'Total Difference','differencep' : 'Total Price Difference'};
            this.printBanner(data,ddic);
        }
        ...
        {
            await this._rpc({
                model: 'clearance.stockin.landedcost',
                method: 'getSummary2',
                args: [[]],           // empty recordset
            }).then(function(runtimeText) {
                data=runtimeText;
        ...
        if (state.model==="clearance.stockout.manual")
        {
            await this._rpc({
                model: 'clearance.stockout.manual',
                method: 'getSummary2',
                args: [[]],           // empty recordset
            }).then(function(runtimeText) {
                data=runtimeText;
        ...
        if (state.model==="clearance.stockin.manual.journal")
        {
            await this._rpc({
                model: 'clearance.stockin.manual',
                method: 'getSummary2',
